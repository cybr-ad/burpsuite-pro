...
#
#  KEY FIX: Block PortSwigger's activation servers via /etc/hosts
#  so BurpSuite CANNOT do online activation and is FORCED to show
#  "Manual Activation" — which is required for the keygen to work.
#
#  Without this block, BurpSuite tries online activation with the
#  keygen's fake license → it silently fails → Manual Activation
#  never appears → activation is impossible.
# ════════════════════════════════════════════════════════════════════
step "STEP 03 ── Activation"

cd "$BURP_DIR"

# ── Block PortSwigger activation servers ─────────────────────────
HOSTS_FILE="/etc/hosts"
HOSTS_MARKER="# BURP-SETUP-BLOCK"
BLOCK_DOMAINS=(
    "portswigger.net"
    "www.portswigger.net"
    "license.portswigger.net"
    "activation.portswigger.net"
    "api.portswigger.net"
)

block_activation() {
    info "Blocking PortSwigger activation servers (temporary)..."
    # Backup hosts
    cp "$HOSTS_FILE" "${HOSTS_FILE}.burp-backup"
    echo "" >> "$HOSTS_FILE"
    echo "$HOSTS_MARKER — START (added by burpsuite_setup.sh)" >> "$HOSTS_FILE"
    for domain in "${BLOCK_DOMAINS[@]}"; do
        echo "127.0.0.1  $domain" >> "$HOSTS_FILE"
    done
    echo "$HOSTS_MARKER — END" >> "$HOSTS_FILE"
    ok "PortSwigger servers blocked → Manual Activation will appear."
}

unblock_activation() {
    info "Restoring /etc/hosts (unblocking PortSwigger)..."
    if [ -f "${HOSTS_FILE}.burp-backup" ]; then
        cp "${HOSTS_FILE}.burp-backup" "$HOSTS_FILE"
        rm -f "${HOSTS_FILE}.burp-backup"
    else
        # Fallback: remove our lines
        sed -i "/$HOSTS_MARKER/d" "$HOSTS_FILE"
        for domain in "${BLOCK_DOMAINS[@]}"; do
            sed -i "/127\.0\.0\.1.*$domain/d" "$HOSTS_FILE"
        done
    fi
    ok "PortSwigger servers unblocked."
}

# Ensure cleanup on exit
trap 'unblock_activation 2>/dev/null' EXIT

# Block before launching keygen
block_activation

echo ""
echo -e "${BOLD}${YELLOW}IMPORTANT — Read before pressing ENTER:${NC}"
echo ""
echo -e "  ${CYAN}The keygen will open now. Here's exactly what to do:${NC}"
echo ""
echo -e "  ${CYAN} 1.${NC}  Copy the ${BOLD}License Key${NC} from the keygen."
echo -e "  ${CYAN} 2.${NC}  Click ${BOLD}[ Run ]${NC} in the keygen to launch BurpSuite."
echo -e "  ${CYAN} 3.${NC}  In BurpSuite: click ${BOLD}Next${NC}."
echo -e "  ${CYAN} 4.${NC}  Paste the License Key → click ${BOLD}Next${NC}."
echo -e "  ${CYAN} 5.${NC}  Online activation will ${RED}${BOLD}FAIL${NC} (this is expected!)."
echo -e "  ${CYAN} 6.${NC}  You will see ${GREEN}${BOLD}Manual Activation${NC} — click it."
echo -e "  ${CYAN} 7.${NC}  Copy the ${BOLD}Activation Request${NC} from BurpSuite."
echo -e "  ${CYAN} 8.${NC}  Paste it into the ${BOLD}Activation Request${NC} box in the Keygen."
echo -e "  ${CYAN} 9.${NC}  Keygen shows ${BOLD}Activation Response${NC} — copy it."
echo -e "  ${CYAN}10.${NC}  In BurpSuite: click ${BOLD}Paste Response${NC} → paste → ${BOLD}Next${NC}."
echo -e "  ${CYAN}11.${NC}  ${GREEN}${BOLD}Done! BurpSuite is activated.${NC}"
echo ""
echo -e "${YELLOW}Press ENTER to launch keygen...${NC}"; read -r

info "Launching BurpLoaderKeygen as: $REAL_USER"
run_as_user java -jar "${BURP_DIR}/${KEYGEN}"

echo ""
echo -e "${BOLD}${GREEN}Is BurpSuite activated?${NC}"
echo -e "\n${YELLOW}Press ENTER when activation is complete...${NC}"; read -r

# ── Unblock servers now that activation is done ──────────────────
unblock_activation
trap - EXIT

# ── Second run: get the loader command ───────────────────────────
echo ""
echo -e "${BOLD}${YELLOW}Now opening the keygen AGAIN to get the Loader Command.${NC}"
echo -e "Look for the ${BOLD}Loader Command${NC} field, copy it, then close keygen."
